...
const blog = toRaw(blogData.value);
...
const formSchema = z.object({
  ...
  author: z.enum(authorsNames).default(blog.author),
  status: z.enum(["published", "unpublished"]).default(blog.status),
});
...
function onSubmit(values) {
  ...
  $fetch(`${config.public.apiBaseUrl}/api/blogs/${blogId}`, {
    method: "put",
    headers: {
      "x-key": config.public.apiKey,
    },
...
<template>
  ...
  <div class="flex flex-col gap-5 max-w-screen-md mx-auto w-full">
    ...
    <div class="flex justify-center w-full">
      ...
      >
        <Button type="submit" class="font-extrabold"> Update Blog </Button>
      </AutoForm>
    </div>
  </div>
</template>
